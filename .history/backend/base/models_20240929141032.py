...
class Product(models.Model):
    ...
    brand = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="Marka"
    )
    category = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="Kategoria"
    )
    ...
    count_in_stock = models.IntegerField(
        null=True, blank=True, default=0, verbose_name="Stan"
    )
...
class TrainingSession(models.Model):
    ...
    status = models.CharField(
        max_length=50, choices=STATUS_CHOICES, default="zaplanowany"
    )
    ...
    def __str__(self):
        ...
        return f"Sesja z {self.trainer} dnia: {formatted_date}"

    def complete_if_due(self):
        
        now = timezone.now()
        
        duration_timedelta = timezone.timedelta(minutes=self.duration)
        
        session_end_time = self.date + timezone.timedelta(minutes=self.duration)
        
        
        if self.status == "planned" and now >= session_end_time + timezone.timedelta(
            minutes=1
        ):
