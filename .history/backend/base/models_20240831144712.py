...
class TrainingSession(models.Model):
    ...
    trainer = models.ForeignKey(Trainer.id, on_delete=models.SET_NULL, null=True)
    date = models.DateTimeField()
    duration = models.DurationField()
    status = models.CharField(max_length=50, choices=STATUS_CHOICES, default='zaplanowany')
    
    def clean(self):
    # Validation of session
        if not (6 <= self.date.hour <= 23):
            raise ValidationError("Trening może się rozpocząć tylko w godzinach 6:00 - 23:00.")
    ...
    def complete_if_due(self):
        """Set status on 'completed' if session is completed"""
        now = timezone.now()
        session_end_time = self.date + self.duration
        if self.status == 'planned' and now >= session_end_time + timezone.timedelta(minutes=1):
